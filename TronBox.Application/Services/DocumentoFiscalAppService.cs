...
namespace TronBox.Application.Services
{
    public class DocumentoFiscalAppService : IDocumentoFiscalAppService
    {
        ...
        public void CancelarDocumentos(IEnumerable<string> chavesCancelamento)
        {
            ...
            var chavesDocNaoExistes = chavesCancelamento.Except(documentosExistentes.Select(d => d.ChaveDocumentoFiscal))
                .ToList();

            if (chavesDocNaoExistes.Any())
            {
                var repository = _repositoryFactory.Instancie<IChaveDocumentoCanceladoRepository>();

                var chavesDb = repository.BuscarTodos(c => chavesDocNaoExistes.Contains(c.ChaveDocumentoFiscal));
                var chavesInserir = chavesDocNaoExistes.Except(chavesDb.Select(c => c.ChaveDocumentoFiscal))
        ...
        private DocumentoFiscalDTO ObterNotaFiscalFromObject(string inscricaoEmpresa, nfeProc nfe)
        {
            ...
            {
                DataArmazenamento = UtilitarioDatas.ConvertToIntDateTime(DateTime.Now),
                DataEmissaoDocumento = UtilitarioDatas.ConvertToIntDate(nfe.NFe.infNFe.ide.dEmi) == UtilitarioDatas.ConvertToIntDate(default) // Validação data default
                ? UtilitarioDatas.ConvertToIntDate(nfe.NFe.infNFe.ide.dhEmi.UtcDateTime) : UtilitarioDatas.ConvertToIntDate(nfe.NFe.infNFe.ide.dEmi),
